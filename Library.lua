...
do
	...
	function Funcs:AddToggle(Idx, Info)
		if Library.ForceCheckbox or (not Info.Looped and Library.ForceNonLoopedCheckbox) then
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		local ArrayValueMetatable =  setmetatable({}, {
			__newindex = function(ValueTable, Index, BooleanValue)
				...
				if BooleanValue then
					table.insert(ValueTable, Index)
				elseif ExistingIndex then
					table.remove(ValueTable, ExistingIndex)
				end
			end,
			__index = function(ValueTable, Index)
				return table.find(ValueTable, Index) and true
			end,
		})

		local Dropdown = {
			Text = typeof(Info.Text) == "string" and Info.Text or nil,
			Value = if Info.Multi then Info.Array and ArrayValueMetatable or {} else nil,
			Values = Info.Values,
		...
		function Dropdown:BuildDropdownList()
			...
			for _, Value in Values do
				...
				local Selected
				if Info.Multi then
					Selected = Dropdown.Value[Value] or (table.find(Dropdown.Value, Value) and true)
				else
...
function Library:Notify(...)
	...
	if typeof(Info) == "table" then
		...
		Data.SoundId = Info.SoundId or (Library.DefaultNotifyData or {}).SoundId
	...
	do
		...
		if NotificationCooldowns[Description] then
			return
		end

		NotificationCooldowns[Description] = true

		task.delay(Data.Time + .5, function()
			NotificationCooldowns[Description] = nil
		end)
