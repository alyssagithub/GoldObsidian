...
do
	...
	function Funcs:AddToggle(Idx, Info)
		if Library.ForceCheckbox or (Library.ForceNonLoopedCheckbox and not Info.Looped) then -- ForceNonLoopedCheckbox
		...
		function Toggle:SetValue(Value)
			...
			if Toggle.Looped then
				local function LoopCallback(Func: (...any?) -> ...any?)
	...
	function Funcs:AddDropdown(Idx, Info)
		...
		local Dropdown = {
			...
			ResetAfterSelection = Info.ResetAfterSelection,

			FormatDisplayValues = Info.FormatDisplayValues or function(Value)
				return Value
			end,
			Callback = Info.Callback,
			Changed = Info.Changed,
		...
		function Dropdown:Display()
			...
			if Info.Multi then
				local function AddToString(Value)
		...
		local function RunDropdownCallback()
			...
			if Dropdown.ResetAfterSelection then
				Dropdown:SetValue()
				MenuTable:Close()
			end
		...
		function Dropdown:BuildDropdownList()
			...
			for _, Value in Values do
				...
				local Button = New("TextButton", {
					...
					Size = UDim2.new(1, 0, 0, 21),
					Text = tostring(Dropdown.FormatDisplayValues(Value)),
					TextSize = 14,
					TextTransparency = 0.5,
					TextXAlignment = Enum.TextXAlignment.Left,
					Parent = MenuTable.Menu,
		...
		function Dropdown:SetValue(Value)
			...
			if not Dropdown.Disabled and Value then
				RunDropdownCallback()
			end
		end
		...
		if next(Defaults) then
			for i = 1, #Defaults do
				...
				if Info.Multi then
					if Info.Array then
						table.insert(Dropdown.Value, Dropdown.Values[Index])
					else
...
function Library:CreateWindow(WindowInfo)
	...
	function Window:SetSidebarWidth(Width)
		Width = math.clamp(Width, 0, MainFrame.Size.X.Offset - WindowInfo.MinContainerWidth - 1) -- leave 2nd arg as 0
